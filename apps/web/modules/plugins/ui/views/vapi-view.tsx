...
const getIntegrationErrorMessage = (error: unknown) => {
  const message = error instanceof Error ? error.message : String(error);

  if (message.includes("VAPI_AUTH_FAILED")) {
    return "Vapi rejected the private API key. Check the key and try again.";
  }

  if (message.includes("AWS_ACCESS_DENIED")) {
    return "AWS Secrets Manager permission denied. Update the IAM policy for this AWS user.";
  }

  return "Could not connect Vapi. Please check your keys and AWS settings.";
};
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const result = await upsertSecret({
        service: "vapi",
        value: {
          publicApiKey: values.publicApiKey.trim(),
          privateApiKey: values.privateApiKey.trim(),
        },
      });
      ...
      console.error(error);
      toast.error(getIntegrationErrorMessage(error));
    }
  };
...
const VapiPluginRemoveForm = ({
  ...
  const removePlugin = useAction(api.private.plugins.remove);
